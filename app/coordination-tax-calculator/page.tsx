...
const IGNITE_ANNUAL = 54000;
const BUILD_ANNUAL = 78000;
const SCALE_ANNUAL = 102000;
...
export default function CalculatorPage() {
  ...
  const results = useMemo(() => {
    ...
    return { toolsCount, frictionMultiplier, weeklyCoordinationHours, weeklyTax, annualTax, recoverableAmount, severity, isCapped, recommendedTier, recommendedTierCost, actualROI };
  ...
  const getSeverityContent = () => {
    ...
    if (severity === 'Severe') {
      return {
        ...
        body: isCapped ? 'Your calculated coordination tax exceeds $2,000,000 per year, and at that scale the problem is not something a calculator can size correctly. What you are experiencing is structural misalignment between how your business has scaled and how your systems are architected to support it. This is exactly the situation LVRGWRKS was built to fix.' : 'At this scale, the coordination tax is not just expensive. It is actively constraining your ability to grow. You cannot fix this with another software subscription or another admin hire. It requires structural redesign of how the work flows across your operation.',
    ...
    if (severity === 'High') {
      return {
        ...
        tierRecommendation: recommendedTier === 'Scale' ? `The Scale tier ($8,500/month, ${formatCurrency(SCALE_ANNUAL)} annually) is sized for your situation.` : `The Build tier ($6,500/month, ${formatCurrency(BUILD_ANNUAL)} annually) fits your current coordination tax.`,
        roiMath: `Your total coordination tax is approximately ${formatCurrency(annualTax)} per year. Approximately ${formatCurrency(recoverableAmount)} is recoverable. A ${recommendedTier} tier engagement (${formatCurrency(recommendedTierCost)} annually) delivers approximately ${formatROI(actualROI)} ROI in the first year.`,
      };
    }
    if (severity === 'Moderate') {
      return {
        ...
        tierRecommendation: recommendedTier === 'Build' ? `The Build tier ($6,500/month, ${formatCurrency(BUILD_ANNUAL)} annually) is sized for your situation.` : `The Ignite tier ($4,500/month, ${formatCurrency(IGNITE_ANNUAL)} annually) is the right entry point.`,
        roiMath: `Your total coordination tax is approximately ${formatCurrency(annualTax)} per year. Approximately ${formatCurrency(recoverableAmount)} is recoverable. An ${recommendedTier} tier engagement (${formatCurrency(recommendedTierCost)} annually) delivers approximately ${formatROI(actualROI)} ROI in the first year.`,
      };
    }
    return {
      label: 'Low',
      ...
      tierRecommendation: `The Ignite tier ($4,500/month, ${formatCurrency(IGNITE_ANNUAL)} annually) is the right entry point.`,
      roiMath: actualROI >= 1 ? `Your total coordination tax is approximately ${formatCurrency(annualTax)} per year, with approximately ${formatCurrency(recoverableAmount)} recoverable. An Ignite tier engagement delivers approximately ${formatROI(actualROI)} ROI in year one.` : `At your current coordination tax, the direct ROI math on Ignite is modest, but the real value is preventing the problem from compounding.`,
    };
  };
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    if (!company.trim()) { setEmailError('Please enter your company name.'); return; }
    ...
    try {
      const response = await fetch('https://formspree.io/f/xvzdzrlr', {
        ...
        body: JSON.stringify({
          ...
          office_staff: officeStaff, industry,
      ...
      else { setEmailError('Something went wrong. Please try again or email us directly at jredgate@lvrgwrks.com.'); }
    } catch { setEmailError('Something went wrong. Please try again or email us directly at jredgate@lvrgwrks.com.'); }
            ...
            <div className="bg-brand-surface p-8 md:p-12 rounded-xl border border-gray-100 mb-12">
              ...
              <div className="mb-10">
                ...
                <div className="flex items-center gap-3">
                  <input type="number" min="5" max="50" value={officeStaff} onChange={(e) => setOfficeStaff(Math.max(5, Math.min(50, Number(e.target.value) || 25)))} className="flex-1 px-4 py-3 rounded-lg border-2 border-gray-200 bg-white text-brand-navy font-bold text-xl focus:border-brand-navy focus:outline-none transition-all" />
              ...
              <div className="mb-10">
                ...
                <select value={industry} onChange={(e) => setIndustry(e.target.value as Industry)} className="w-full px-4 py-4 rounded-lg border-2 border-gray-200 bg-white text-brand-navy font-medium focus:border-brand-navy focus:outline-none transition-all">
              ...
              <div className="mb-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {TOOL_OPTIONS.map((tool) => (
                    <button key={tool} onClick={() => toggleTool(tool)} className={`px-4 py-3 rounded-lg text-sm text-left transition-all border-2 ${selectedTools.includes(tool) ? 'bg-brand-navy text-white border-brand-navy' : 'bg-white text-brand-slate border-gray-200 hover:border-brand-navy/30'}`}>
                      {tool}
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input type="number" min="30" max="200" value={hourlyCost} onChange={(e) => setHourlyCost(Math.max(30, Math.min(200, Number(e.target.value) || 75)))} className="flex-1 px-4 py-3 rounded-lg border-2 border-gray-200 bg-white text-brand-navy font-bold text-xl focus:border-brand-navy focus:outline-none transition-all" />
                ...
                <div className="bg-brand-navy text-white p-8 md:p-12 rounded-xl mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                    ...
                    <div className="bg-white/5 border-2 border-brand-gold p-6 rounded-xl">
                      <div className="text-brand-gold font-display font-bold text-3xl md:text-4xl mb-2">{results.isCapped ? '$500K+' : formatCurrency(results.annualTax)}</div>
                  ...
                  <div className="border-t border-white/10 pt-8">
                    ...
                    <button onClick={() => openBriefing()} className="w-full sm:w-auto bg-brand-gold text-brand-navy px-10 py-4 rounded-lg font-bold text-sm uppercase tracking-widest hover:bg-white transition-all shadow-xl">
                  ...
                  {emailSubmitted ? (
                    ...
                    <>
                      ...
                      <form onSubmit={handleEmailSubmit} className="space-y-4">
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" required className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-white text-brand-navy font-medium focus:border-brand-navy focus:outline-none transition-all" />
                        <input type="text" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Company name" required className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-white text-brand-navy font-medium focus:border-brand-navy focus:outline-none transition-all" />
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" required className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 bg-white text-brand-navy font-medium focus:border-brand-navy focus:outline-none transition-all" />
                        <button type="submit" disabled={emailSubmitting} className="w-full bg-brand-navy text-white px-8 py-3 rounded-lg font-bold text-sm uppercase tracking-widest hover:bg-brand-slate transition-all shadow-lg disabled:opacity-60">
      ...
      <section className="py-20 md:py-32 bg-brand-navy text-white text-center">
        <div className="container mx-auto px-6 md:px-16">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6 items-center">
            ...
            <Link href="/fractional-cto-calgary" className="text-white text-sm font-bold uppercase tracking-widest border-b-2 border-brand-gold pb-1 hover:text-brand-gold transition-all">
